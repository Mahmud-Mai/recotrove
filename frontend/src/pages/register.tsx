...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_API_URL || "http://localhost:8030/api/v1"}/auth/register`,
        {
          email,
          full_name: fullName,
          password,
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid gap-4">
              <div className="grid gap-1">
                <label className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 sr-only" htmlFor="fullName">
                  Full Name
                </label>
                <input
                  id="fullName"
